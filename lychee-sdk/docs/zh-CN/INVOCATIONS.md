...
[目录](README.md) · [English](../en/INVOCATIONS.md)
...
`schema` 是参数名到字段规则的纯数据映射。所有类型可声明 `required`、`default`、`unit`；明确默认值在参数规范化时展开，`false` 是合法默认值。未知字段、函数、metatable、循环、secret/不可访问值及非有限数被拒绝，输入不被修改。
