...
[目录](README.md) · [English](../en/PROTOCOLS.md)

公开 API 版本：API_VERSION="1.0.0"。SDK 发行版本同为 1.0.0，UI Runtime 版本 1。API 使用完整版本字符串；未知字段会被拒绝。接入步骤见[教程](GETTING_STARTED.md)。

## 可选能力与独立加载

本版仍为 1.0.0。`SupportsFeature(name,1)` 对未知能力返回 false；客户端快照、发现/就绪、准备、Invocation，以及search-documents、compact-storage、query-failure合同分别检查。加载前声明见 [LOADING](LOADING.md)，参数化动作、StoredRef 与面板上下文见 [INVOCATIONS](INVOCATIONS.md)，存储所有权见 [STORAGE](STORAGE.md)。第三方不需要 Host 源码或项目 Manifest。
...
| apiVersion | 必填字符串 `"1.0.0"`，与 SDK 发行版本一致。 |
...
| addon / resource | 冷声明来源必须给出真实包名与自有资源描述，并与 TOC 一致；详见 [加载契约](LOADING.md)。 |
| prepare | 可选搜索准备回调，准备什么由 Provider 决定，截止与取消由 Host 调度。 |
| describe / resolveTarget / observe / targetView | 可选参数目标描述、异步精确目标解析、值观察与显式目标面板；只读准备不能写业务。见 [Invocation](INVOCATIONS.md)。 |
...
普通小来源可提供 `entries`（最多 4096 条）而不写 query；需要动态查询时提供 query，可与 entries 组合。`searchable=false` 表示静态目录不参加通用索引；`searchMode` 控制原有静态匹配入口。这些声明不替代用户的参与搜索偏好。Host 只索引公开有界条目，不取得业务 DB。
...
| SetAvailability(boolean,reason?) | 修改运行时可用性，不覆盖用户选择。reason 为可选字符串，最多 256 字节；恢复可用时清除原因。非法原因返回错误，不能改变原状态。 |
...
句柄保留 `Update({replace=entries})` 或 `Update({upsert=entries,remove=ids})`，校验成功后原子更新普通目录。`Settings()` 是现有有界角色设置入口，不用于大目录或第三方业务 DB；第三方业务存储见 [Storage](STORAGE.md)。不提供旧名称 `SetEnabled`，所有者使用 `SetAvailability`；用户开关由 Host 管理。

用户的“参与搜索”开关不改变句柄业务可用状态，也不调用 `onDisable`；明确恢复、业务页面和已提交操作仍有效。
...
| 字段 | 类型与约定 |
|---|---|
| id | 必填 string，1–128 字节，ASCII 字母/数字起始，随后可含 `._:/-`。普通目录内稳定唯一。动态候选按完整规范引用唯一；同ID不同Invocation参数可以并存。 |
| title | 必填非空Text；本地化解析后仍须有非空标题。 |
...
最近使用最多八个有界稳定引用：兼容 `{providerID,entryID}`，也支持带 product、目标和参数的 `target`、`command`、`invocation` 引用。旧条目通过同步 resolve 精确恢复；新引用通过 resolveTarget 与 Invocation 准备恢复，可异步，必须继承剩余截止时间。恢复不是模糊搜索，也不执行业务写入。临时不可恢复条目或 rememberable=false 不进入历史；完整结构见 [Invocation](INVOCATIONS.md)。

取消函数最多调用一次，包含正常完成、换词、关闭、超时、错误和启停原因；完成后仍要清理自己的临时任务。同步 Lua 不能被抢占。查询协议和评分见上文及 [Catalog](CATALOG.md)。

## 补充结果字段

`rememberable?:boolean`，false 表示不进入固定/最近使用；传统条目提供 resolve，新引用提供相应目标解析与动作/面板能力。`tooltipRows?:string[][]` 最多 16 行，每行最多 3 列，每列最多 512 字节；只作为通用提示展示，不包含钥匙等具体业务字段。
