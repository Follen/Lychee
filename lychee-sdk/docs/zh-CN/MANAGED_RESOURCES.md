# SDK 1.0.0：托管资源、设置与缓存

[目录](README.md) · [English](../en/MANAGED_RESOURCES.md)

性能、容量与生命周期预算统一见[性能硬门禁](PERFORMANCE.md)；本页说明接口使用方式。

检查 `Lychee:Supports("1.0.0")`，Provider 声明 `apiVersion="1.0.0"`。公共普通记录可由 entries/Update 或 query 提交。运行时只依赖 `_G.Lychee`，不得使用 `LycheeInternal`。
...
业务设置使用 [SDK Storage](STORAGE.md) 绑定子插件自己的 SV 根，Host 不提供业务 DB。存储句柄的生命周期由子插件管理：

- `Get(key, default?)`：返回副本；不存在时返回 default 的副本，不自动落盘。
- `Set(key, value)`：保存副本，nil 删除该设置；失败不覆盖旧值。
- `Import(values)`：整份设置原子导入尚不存在的命名空间；已有目标拒绝覆盖。
- 禁用和关闭不删除设置。子插件负责在不再使用句柄时调用 `Close()`；Provider 注销不会自动关闭子插件共享的数据层。同 ID 新实例重新打开后可读取已保存的选择。
- 角色/账号范围由子插件自己的 TOC 和数据根决定，荔枝业务设置按角色；SDK 不自动扫描或迁移第三方数据库。设置保存后如何更新业务目录由 Provider 决定。
...
- 调用方的 Catalog:Update 保留原子提交、失败保留旧目录、无变化不通知的契约。SDK Catalog 由调用方持有；普通 Provider.entries/Update 仍允许 Host 索引有界公开条目或 SearchDocument，不等于接管 Provider 业务数据库。
...
搜索与详情的数据生命周期应分开：扫描借用当前query独占的具名临时记录，只有入选候选才复制必要字段；打开详情后才创建完整对象。临时记录可以由所属协程跨批次使用，但不能交给其他query或被已发布结果持有。取消测试应验证挂起协程捕获的记录也能回收。不要在每次输入时重建完整资料库，再用关闭后的GC数字宣称低成本。
