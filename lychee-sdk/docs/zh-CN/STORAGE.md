...
[目录](README.md) · [English](../en/STORAGE.md)
...
当前单插件合同不承诺 `RegisterMigrationOwner`、`GetMigrationOwner` 或 `RequestMigrationOwner` 公开 API。十包版迁回本分支时，先备份实际存档，再根据明确的来源 ID/schema 制定一次性迁移；目标验证成功之前保留原数据。必须保留用户设置。
