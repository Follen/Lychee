...
发行版本 1.0.0；API_VERSION=3、API_REVISION=1、helper 默认 MIN_API_REVISION=1。当前不兼容 API 2。编辑器声明 ApiStubs.lua 不进入 AddOn TOC。
...
测试必须分别破坏 Host、类型、helper、文档、交付文件和版本下限，验证单边漂移会失败。不能仅搜索版本字符串，也不能修改输入补齐必填字段后宣称公开边界通过。
