...
索引集合只有一个条目时直接保存条目键，多条目才分配集合；移除后可缩回单条目。每条记录不再常驻保存逐个前缀/字片段的反向关系小表及第二份去重表，删除时只从该记录的编译字段重算关系。坐骑变更检测缓存仅保留名称、图标和法术 ID，动作描述由 Provider 持有。边界拷贝隔离保持不变。内存基线、性能取舍与回归门槛见 [搜索内存优化记录](validation/2026-09-10-search-memory.md)。
