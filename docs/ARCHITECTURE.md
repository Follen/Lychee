...
HomeView 当前只显示最近使用记录；空状态不伪造实体。记录标题、图标和 Source/Provider 提供的元数据由 Palette 投影到 Home tile，输入路径不得创建 Frame 或重扫完整索引。分类和来源过滤仍由 SearchSession/StaticIndex 的结构化 filter 能力保留给后续 Home 视图，不把显示标题伪装成普通文本查询。
