...
结果行保持紧凑；Host tooltip 依次包含 canonical 标题、Source/Provider 提供的 `kindTitle`（缺省时显示稳定的 `kind` 标识）、描述/副标题、Source 或 Extension 提供的来源标题、匹配证据（字段、匹配类型、置信度）和已声明动作。Host 不维护实体类型或来源 ID 到文案的字典。第三方只提交 plain-data 字段，不创建或接管 `GameTooltip`；空字段省略，禁止写入 Enter、方向键、Esc 教学、异常堆栈、secret/inaccessible 值或未声明动作。
