...
- 改动不增加 WoW API；内存计量定义和既有坐骑详情 API 的 path/line/excerpt 见 [版本证据](../design/2026-09-10-search-memory-wowdoc.json)。例如 `Interface/AddOns/Blizzard_APIDocumentationGenerated/PerformanceDocumentation.lua:94` 定义 UpdateAddOnMemoryUsage；坐骑详情沿用 `MountJournalDocumentation.lua:273`。
- `wowdoc validate --path package/Lychee --source wow-ui-source --product retail --ref latest` 检查 37 个运行时 Lua，valid=true、零诊断，见 [校验结果](../design/2026-09-10-search-memory-validate.json)。
