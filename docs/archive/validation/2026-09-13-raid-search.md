...
精确首领/副本/ID先返回；其余每128技能组/1ms检查让出，有界Top20。查询作用域拥有扫描、待加载集合、事件和deadline；最多等待1.5秒后最后重扫。同步加载、失败、取消、禁用、战斗都收尾。无新增空闲OnUpdate、SV或全量技能名索引。所有语言的查询都使用托管资源。
...
运行提交 `87f2268e3c7f8d855541eb7ae77e787b1210fce1` 已同步到正式服 Lychee；发布清单142文件逐项SHA256相同，20个旧文件保留，未变更TOC或新增模块，已/reload载入。

Ticket `LYCHEE-20260913-024212-0021`，source kind `automation_result`，retail12.1.0/69587/zhCN，request `req-20260913-raid-search-02` / `search-v2`。报告1053字节，SHA256 `f4be2e52315abd1527aaba824fbfa4a4d76a507b407a12fafe4d8e533f6c0f13`，succeeded/complete，无截断，已读回并ACK。四个真实Host查询0.828秒完成：

| 搜索 | 首领 | 技能 | 难度 | section | 延迟秒 |
| --- | --- | --- | --- | --- | --- |
| 团本首领：盘魂者内克扎莉 | 2888 | 首领入口 | 默认 | 默认 | 0 |
| 团本首领：普通 盘魂仪式 | 2888 | 1284033 | 14 | 36048 | 0.280 |
| 团本首领：英雄 盘魂仪式 | 2888 | 1284033 | 15 | 36540 | 0.261 |
| 团本首领：史诗 盘魂仪式 | 2888 | 1284033 | 16 | 35982 | 0.270 |

查询完成后handle诊断resources=0、providerResources=0；同时断言中文两个Provider实际标题正确。此样本为已运行客户端，不作为全冷名称加载、动作点击或帧率压力证据。准确原生打开参数与战斗限制由离线测试覆盖，尚未逐个技能实机点击。

首次临时探针误把搜索前缀ldt当内部ID，Ticket `LYCHEE-20260913-024120-0020` 在执行搜索前失败；探针改为读取模块实际ID后得到以上成功结果，游戏代码未因此修改。两次报告都ACK，本次两个任务块已通过工具删除，不保留后台诊断任务。

完整测试复跑中新增查询最大回调5ms，仍低于8ms门槛；本文先前2ms为针对性测试观测，并非全程最大值保证。所有耗时均为本机样本。
