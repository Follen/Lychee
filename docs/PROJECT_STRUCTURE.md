...
| 位置 | 放什么 |
| --- | --- |
| `package/Lychee/` | 游戏运行时；只从这里交付 |
| `package/Lychee/Builtin/<功能>/` | 该功能的实现、独立语言资源和专用数据 |
| `package/Lychee/Builtin/Shared/` | 内置功能共用的目录刷新、支持范围读取和界面动作 |
| `package/Lychee/Builtin/Definitions.lua` | 工具生成的支持声明，不手改 |
| `package/Lychee/Builtin/Init.lua` | 按生成声明启动功能，不维护另一张功能名单 |
| `package/Lychee/Core/`、`Search/` | Host 的注册、调度、搜索与记录生命周期 |
| `package/Lychee/UI/`、`Secure/` | 界面交互与受保护动作执行 |
| `package/Lychee/PublicAPI/SDK.lua` | 游戏里真正使用的公开入口，必须保留 |
| `package/Lychee/Locales/` | Host 界面文案；不收纳 Provider 文案 |
...
6. 运行 `powershell -NoProfile -File tests/check_contract.ps1`；加载文件变化后需要重启游戏客户端。
...
以后需要按 interface/build 区间选择多个实现时，先按 [客户端差异约定](../lychee-sdk/CLIENT_VARIANTS.md) 明确唯一匹配、稳定身份、缓存版本和清理规则，再扩展构建清单与验证；当前清单不接受未经实现的新字段。不要把内部 Support 当作第三方 SDK。
