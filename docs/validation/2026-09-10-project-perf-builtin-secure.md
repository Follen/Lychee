...
计时总量为 100 次刷新，不是单次耗时；Lua os.clock 分辨率有限。最终三轮前后交替测量原始输出在 `docs/architecture/2026-09-10-project-perf-builtin-results.json`。环境扰动使 CPU 数字只具方向意义。累计分配在受控 GC stop 测量，结束恢复；不代表峰值或游戏插件内存。启动与真实单批峰值待游戏测量。
...
- 精确技能书 API、事件及框架证据见 `docs/architecture/2026-09-10-project-perf-builtin-api.json`；计时器、异步结果证据见同前缀 timer / spell-load JSON。
- `RegisterUnitEvent` / `UnregisterEvent` / 玩家施法与脱战事件版本证据见 `docs/architecture/2026-09-10-project-perf-builtin-secure-events.json`。
