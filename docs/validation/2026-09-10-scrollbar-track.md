...
Lua/XML/TOC 静态检查、wowdoc validate（45 Lua，valid=true）、git diff --check 通过。完整契约中其他项目通过，背包冷启动首次 CPU 53 ms / peak 3.36 ms 超预算；保留原始失败日志，独立复测 Provider 与后续 Motion 通过，未放宽门槛。见 scrollbar-track-checks.txt、scrollbar-track-recheck.txt。该计时波动发生于未改动的 Provider 路径，不作为样式性能收益。
