...
实机性能已完成，见下方最终验收；最终提交的独立功能覆盖探针仍待运行。上一轮 14.032 MiB 的 Ticket 对应旧提交和不同职业，不能作为本轮收益证据。真实动作点击、战斗及英文客户端不因离线测试通过而自动视为通过。
...
本次输入与重载由用户手工完成；结果已读出，客户端 received 确认待手工执行。最终提交的独立功能覆盖探针仍待运行；本轮已验证搜索结果等价，不代表真实动作点击、战斗或英文客户端全部通过。
