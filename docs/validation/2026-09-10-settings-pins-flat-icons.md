...
- `wowdoc validate --path package/Lychee --source wow-ui-source --product retail --ref latest`: valid, 39 Lua files, no diagnostics. Evidence: `docs/architecture/2026-09-10-settings-pins-validate.json`.
- Pre-edit source evidence: `wow-ui-source`, retail, requestedRef `latest`, resolved commit `8ea15b61e45c0ed4eba01439c90757f86eb78d34`. Exact paths, lines and excerpts for setters/drag/special-frame behavior are retained in `docs/architecture/2026-09-10-settings-pins-wowdoc.json`.
- Reproducible icon export: `tests/build_flat_menu_icons.cjs`; source/output SHA-256 manifest and dark-background 28/34/48 px preview are under `docs/architecture/2026-09-10-flat-menu-icons.*`. Visual review confirmed all silhouettes remain visible on #101012 and that no image background ships.
