...
当前工具将已核对的 16 个自带 Provider 偏好 ID 从 `lychee.*` 对应到 `builtin.*`；普通引用只接受代码中列出的稳定 ID 模式，参数引用只转换已核对的音量 `set-volume` v1。未知的自带来源、无法证明的条目/动作/目标及冲突保留原文并报告，不猜测替代目标。报告出现 `preserved` 或 `conflict` 必须逐项审查；第三方引用保持原样，不能将“报告没有映射”理解为它已经恢复成功。
