...
插件版本以 `tools/client_manifest.json` 和生成的 TOC 为准；SDK / Provider API 为 1.0.0。运行时唯一来源是 `addon/Lychee`；第三方接入见 [SDK](../../lychee-sdk/docs/zh-CN/GETTING_STARTED.md)，精确字段见 [协议](../../lychee-sdk/docs/zh-CN/PROTOCOLS.md)。

本分支已采用单插件结构，背景见[历史功能迁移计划](../archive/design/2026-09-14-single-addon-feature-port.md)。[架构](../ARCHITECTURE.md)定义现行职责，[生命周期验收入口](../../tests/LIFECYCLE_ACCEPTANCE.md)列出现行测试。运行时仍为单目录，不增加伴随运行时目录或扩大自动同步范围。
...
索引基准可用 `lua tests/performance/index_benchmark.lua` 或 `lua tests/performance/index_benchmark.lua addon/Lychee/Search/StaticIndex.lua delta` 运行。它只测离线核心索引，不包含完整 SDK 校验、游戏 CPU 或帧时间；测量口径见 [性能规范](../../PERFORMANCE.md)。
...
多包开发版到本分支不能只靠覆盖目录完成。先备份并核对原有账号/角色 SV、Provider ID 与具体引用，按[转换交付步骤](DELIVERY.md#从多包开发版转换)隔离旧自带包。Bootstrap 不再因账号 schema 不同而重置整表；各数据所有者仍需校验自己的字段。这不是跨版本迁移保证，来源 ID、业务设置及具体动作引用必须先完成转换核对。用户偏好必须保留。
...
Provider 声明多个客户端或版本区间时，按 [客户端与 build 差异约定](../../lychee-sdk/docs/zh-CN/CLIENT_VARIANTS.md) 为每个实际分支验证选择边界、缺失能力、唯一注册、业务行为、独立 i18n、历史／缓存隔离和启停清理。一个客户端的 API 存在性或测试通过不能代替其他分支。分支的 WoW API 证据必须对应正确的 wowdoc product/ref；离线结果与实机结果分开记录。
