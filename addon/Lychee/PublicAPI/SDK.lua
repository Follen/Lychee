...
function SDK.SupportsFeature(first,second,third)
    ...
    if first==SDK then name,version=second,third end
    ...
    if name=="invocation" then return I.Invocations~=nil end
    if name=="preparation" then return I.Preparation~=nil end
    return false
end
function facade:SupportsFeature(name,version) return SDK.SupportsFeature(name,version) end
local pendingAddons={}
...
local visibilityState,visibilityRevision=false,0
...
function I.NotifyPaletteVisibility(visible)
    if visibilityState==visible then return end
    visibilityState,visibilityRevision=visible,visibilityRevision+1
    local revision=visibilityRevision
    -- A reentrant transition invalidates delivery of the previous state.
    local callbacks={}
    for at,row in ipairs(visibilityListeners) do callbacks[at]=row end
    for _,row in ipairs(callbacks) do
        if revision~=visibilityRevision then break end
        if row.callback then pcall(row.callback,visible) end
    end
end
