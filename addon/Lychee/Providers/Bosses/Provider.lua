...
local L = I.ProviderLocales:Module("builtin.bosses")
local M
...
M=I.ProviderModules.CatalogProvider:New("builtin.bosses",L["团本首领"],{"ADDON_LOADED"},build,
    {open={title=L["查看首领指南"],run=function(entry)
        return I.ProviderModules.InterfaceActions:Run(function()
            return M:Open(entry)
        end,L)
    end}})
...
local function bossMask(id)
    local mask=0
    for text in (I.ProviderModules.JournalCatalog.difficulties[id] or ""):gmatch("%d+") do
...
function M:Open(entry,family)
    ...
    return I.ProviderModules.InterfaceActions:OpenJournal(payload.instanceID,payload.encounterID,payload.difficultyID,payload.sectionID)
end
for family,key in ipairs(familyEnglish) do
    local target=family
    M.actions[key]={title=L[familyNames[family]],run=function(entry)
        return I.ProviderModules.InterfaceActions:Run(function() return M:Open(entry,target) end,L)
    end}
end
...
if I.Locale:IsChinese() then
    function M:Init()
        ...
        local data,records=I.ProviderModules.JournalCatalog,{}
        for offset=1,#data.encounters,3 do
            local encounterID,instanceID,canonical=data.encounters[offset],data.encounters[offset+1],data.encounters[offset+2]
            local instance=data.instances[instanceID]
