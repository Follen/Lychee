...
local actions={
    open={title=L["打开并定位"],run=function(entry)
        local spec=I.ProviderModules.SettingsAdapter.byID[entry.id]
        local result=I.ProviderModules.SettingsAdapter.Open(spec)
...
I.ProviderModules.BlizzardSettings=M
...
function M:onStop() I.ProviderModules.SettingsAdapter.Clear() end
function M:Init()
    if not self.readEntry then
        local function read(id,context)
            ...
            local spec=I.ProviderModules.SettingsAdapter.byID[id]
