...
function Palette:Create()
    ...
    self.input:SetSubmitCallback(function()
        if self.social.active or self.waitingPresentation then return end
...
function Palette:SetFooterText(status, hint)
    if self.retryComponent then self.retryComponent.frame:Hide() end
...
function Palette:PrepareHome(allowExpand)
    if not self.visible or self.waitingPresentation or not self.homeView or self.input.composing then return false end
...
function Palette:SetStatus(mode, count)
    ...
    if mode=="search" and self.searchIncomplete and not self.searchPending then
        self:SetFooterText(text,L["重试"])
        if not self.retryComponent then
            self.retryComponent=Lychee.UI.Components:CreateNavigationButton(self.footer,{
                text=L["重试"],primary=true,width=56,height=28,
                onClick=function(button)
                    local current=button.pressedSession==self.session and button.pressedGeneration==self.generation
                    button.pressedSession,button.pressedGeneration=nil,nil
                    if current then self:RetrySearch() end
                end,
            })
            local button=self.retryComponent.frame
            button:SetPoint("RIGHT",self.footerHint,"RIGHT",0,0)
            Lychee.UI.Theme:SetFont(self.retryComponent.label,"meta")
            button:SetScript("OnMouseDown",function(_,key)
                if key=="LeftButton" then button.pressedSession,button.pressedGeneration=self.session,self.generation end
                self.retryComponent:SetState("pressed")
            end)
        end
        self.footerHint:Hide();self.retryComponent.frame:Show()
    end
end

function Palette:RetrySearch()
    local search=I.Search.Session
    if not self.visible or self.settingsOpen or not self.searchIncomplete or self.searchPending
        or self.input.composing or self.viewHost and self.viewHost:IsActive()
        or InCombatLockdown and InCombatLockdown() or not search then return false end
    if self.activeFilter then return search:Filter(self.activeFilter) end
    local raw=self.input:GetText()
    if raw~=search.raw then return false end
    local ok=search:Input(raw)
    if ok then self:SetQueryMode(raw);self.input:Focus() end
    return ok
end
...
function Palette:ResizeForMode(mode, count)
    ...
    if mode=="search" and self.waitingPresentation then return true end
...
function Palette:FocusInvocationError(row, problem)
    local input, span = self.input, problem and problem.span
    local search = I.Search and I.Search.Session
    if not input or not span or not search or self.settingsOpen or self.waitingPresentation
        or input.composing or not input:IsEnabled() or not self.list or not self.list.frame:IsShown()
        or not self:ValidateRowAction(row) then return false end
    local raw = input:GetText()
    if raw ~= search.raw or #raw > 1024 then return false end
    local first, last = span.start, span.finish
    if type(first) ~= "number" or type(last) ~= "number" or first % 1 ~= 0 or last % 1 ~= 0
        or first < 1 or first > #raw + 1 or last < first - 1 or last > #raw then return false end
    -- Never select half a UTF-8 code point. Native EditBox offsets count bytes.
    local left, right = raw:byte(first), raw:byte(last + 1)
    if left and left >= 128 and left < 192 or right and right >= 128 and right < 192 then return false end
    local box = input.frame
    if not box.HighlightText or not box.SetCursorPosition then return false end
    local item, session, generation = row.item, row.session, row.generation
    input:Focus()
    -- Focus callbacks may navigate or replace the input synchronously.
    if input:GetText() ~= raw or search.raw ~= raw or input.composing
        or not self:ValidateRowAction(row, session, generation, item) then return false end
    box:HighlightText(first - 1, last)
    box:SetCursorPosition(first - 1)
    return true
end
...
function Palette:SetQueryMode(text)
    ...
    if not empty or self.activeFilter then
        if self.waitingPresentation then return true end
    elseif self.waitingPresentation then
        self:CancelWaitingHint();self.waitingPresentation=nil
        self.list:Clear();self.homeView:ReleaseBindings()
    end
...
function Palette:ApplySearchState(session, generation, pending, items, incomplete)
    ...
    if not pending then
        self:CancelWaitingHint()
        if not items and self.waitingPresentation then self:SetFooterText("","") end
    end
    if pending and (not items or #items==0) and self.visible and not self.settingsOpen and not (self.viewHost and self.viewHost:IsActive()) then
        if not (InCombatLockdown and InCombatLockdown()) then self:BeginSearchWaiting() end
        return true
    end
...
function Palette:CancelWaitingHint()
    local timer=self.waitingHintTimer
    self.waitingHintTimer,self.waitingHintVisible=nil,nil
    if timer then timer:Cancel() end
end

function Palette:BeginSearchWaiting()
    if not self.waitingPresentation then
        self.waitingPresentation = true
        Lychee.UI.Motion:StopHeight(false)
        Lychee.UI.Components:HideTooltip(); Lychee.UI.Components:HideActionMenu()
        self.actionMenu, self._deferredHover, self._deferredHoverRevision = nil, nil, nil
        if self.secureBroker then self.secureBroker:ReleaseAll() end
        self._searchActionsSuspended = true
        local hasList = self.list.frame:IsShown() and #self.list.items > 0
        local hasHome = self.homeView:HasPresentation()
        self.list:FreezePresentation(); self.homeView:FreezePresentation()
        if not hasList and not hasHome then
            self.list.frame:Hide(); self.homeView.frame:Hide()
            self.emptyStateComponent:SetTitle("")
            self.emptyState:Show()
            self:ResizeForMode("waiting", 0)
            Lychee.UI.Motion:StopHeight(true)
        end
        self:SetFooterText("", "")
    end
    if self.waitingHintTimer or self.waitingHintVisible then return end
    local function show()
        if not self.visible or not self.searchPending or not self.waitingPresentation or self.settingsOpen
            or self.input.composing or InCombatLockdown and InCombatLockdown() then return end
        self.waitingHintVisible=true
        if self.emptyState:IsShown() then self.emptyStateComponent:SetTitle(L["搜索中…"]) end
        self:SetFooterText(L["搜索中…"], "")
    end
    if C_Timer and C_Timer.NewTimer then
        local timer
        local scheduled
        scheduled,timer=pcall(C_Timer.NewTimer,.12,function()
            if self.waitingHintTimer~=timer then return end
            self.waitingHintTimer=nil;show()
        end)
        if scheduled and timer then self.waitingHintTimer=timer else show() end
    else show() end
end
...
function Palette:ApplyResults(items, generation, session, offset)
    ...
    local wasWaiting = self.waitingPresentation
    self:CancelWaitingHint(); self.waitingPresentation = nil
    self.emptyStateComponent:SetTitle(L[self.searchIncomplete and "部分来源暂不可用，请重新搜索" or "没有找到结果"])
    items = items or {}
