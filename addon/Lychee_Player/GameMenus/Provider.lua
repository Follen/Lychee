local _,I=...
local L = I.ProviderLocales:ForProvider("lychee.game-menus")
local A = I.Modules.InterfaceActions
local M = {}
I.Modules.GameMenus = M
...
function M:Init()
    ...
    records[#records+1]={id="lychee-settings",rememberable=false,title=L["荔枝设置"],kindTitle=L["设置"],
        icon="Interface\\AddOns\\Lychee\\Media\\MenuIcons\\settings.tga",
    ...
    local handle, err = I.Modules.Support:Register({
        ...
        actions={open={title=L["打开界面"],run=function(entry)
            ...
            local result=A:Run(open,L)
            if entry.payload.menuID=="lychee-settings" then result.close=false end
            return result
        end}},
