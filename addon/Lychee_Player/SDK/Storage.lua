...
function Storage.Open(options)
    if type(options)~="table" or not keyOK(options.id) or type(options.root)~="function"
        ...
        or options.migrations~=nil and type(options.migrations)~="table" then return failure("INVALID_SCHEMA") end
    ...
    for step,fn in pairs(options.migrations or {}) do
        if type(step)~="number" or step<1 or step>=version or step~=math.floor(step) or type(fn)~="function" then
