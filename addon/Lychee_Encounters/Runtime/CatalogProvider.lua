...
function C:New(id, title, events, build, actions)
    local ledger=I.Modules.CatalogLedger:New()
    local m = {id=id, locale=I.ProviderLocales:ForProvider(id), title=title, events=events, build=build, actions=actions,
...
function C:Detach(reason)
    ...
    self.dirty=nil
    -- Keep only committed identities while disabled: its private catalog is released.
...
function C:NotifyReady()
    ...
    if self.handle then self.handle:Invalidate() end
end
function C:Queue()
...
function C:Step()
    ...
    if not self.job then
        self.job=coroutine.create(function()
            ...
            coroutine.yield(); count=0; started=now()
            local committed,err=self.ledger:Reconcile(self.handle.catalog,signatures,changedRecords,{
...
function C:Init()
    ...
    self.handle=self.handle or I.Modules.Support:Register({
        ...
        onEnable=function(handle)
            ...
            -- Re-enabling rebuilds this package-owned catalog; re-read and replace
