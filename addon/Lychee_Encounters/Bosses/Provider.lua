...
local function build(self,put,checkpoint)
    ...
    local data=I.Modules.JournalCatalog
...
M=I.Modules.CatalogProvider:New("lychee.bosses",L["团本首领"],{"ADDON_LOADED"},build,
...
local function bossMask(id)
    local mask=0
    for text in (I.Modules.JournalCatalog.difficulties[id] or ""):gmatch("%d+") do
...
function M:ResolveReference(id,family)
    ...
    for offset=1,#data.encounters,3 do if data.encounters[offset]==boss then
        ...
        local total,best,section,previousSpell=0,nil,nil,0
        for sid,sec,maskText in (data.abilities[boss] or ""):gmatch("([0-9a-z]+):([0-9a-z]+):([0-9a-z]+)") do
            local currentSpell=tonumber(sid,data.numberBase)+(data.deltaSpell and previousSpell or 0)
            previousSpell=currentSpell
            if currentSpell==spell then
                local mask=tonumber(maskText,data.numberBase)
                ...
                if diff and (not best or families[diff]<families[best] or families[diff]==families[best] and diff<best) then best,section=diff,tonumber(sec,data.numberBase) end
            end
        end
...
function M:Open(entry,family)
    ...
    return I.Modules.InterfaceActions:OpenJournal(payload.instanceID,payload.encounterID,payload.difficultyID,payload.sectionID)
end
for family,key in ipairs(familyEnglish) do
    local target=family
    M.actions[key]={title=L[familyNames[family]],run=function(entry)
        return I.Modules.InterfaceActions:Run(function() return M:Open(entry,target) end,L)
    end}
end
    ...
    local function work(skills,loadMissing)
        ...
        for offset=1,#data.encounters,3 do
            ...
            if not skills then
                ...
                for spellText,sectionText,maskText in (data.abilities[id] or ""):gmatch("([0-9a-z]+):([0-9a-z]+):([0-9a-z]+)") do
                    local spell,mask=tonumber(spellText,data.numberBase)+(data.deltaSpell and previousSpell or 0),tonumber(maskText,data.numberBase)
                    previousSpell=spell
                    ...
                    if d and (not diff or families[d]<families[diff] or families[d]==families[diff] and d<diff) then diff,section=d,tonumber(sectionText,data.numberBase) end
                    if family then
                        local any=choose(mask)
                        if any and (not fallbackDiff or families[any]<families[fallbackDiff] or families[any]==families[fallbackDiff] and any<fallbackDiff) then fallbackDiff,fallbackSection=any,tonumber(sectionText,data.numberBase) end
                    end
                end
                flush()
            end
...
if I.Locale:IsChinese() then
    function M:Init()
        ...
        local data,records=I.Modules.JournalCatalog,{}
        for offset=1,#data.encounters,3 do
            local encounterID,instanceID,canonical=data.encounters[offset],data.encounters[offset+1],data.encounters[offset+2]
            local instance=data.instances[instanceID]
