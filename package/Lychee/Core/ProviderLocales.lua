...
function P:Builtin(id)
    ...
    local translator,err=self:Compile(resources,"i18n."..tostring(id))
    if not translator then return nil,err end
    translator.resources=resources
    -- No namespace cache: the owning built-in module retains its translator.
    return setmetatable(translator,{__index=function(self,key)
