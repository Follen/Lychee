...
local function talents(_,put,checkpoint)
    ...
    for _,id in ipairs(ids) do
        ...
        put({id="talent:"..id,title=info.name,kind="talent-loadout",kindTitle="天赋方案",
            icon="Interface\\AddOns\\Lychee\\Media\\MenuIcons\\talents.tga",
            subtitle=current and "当前方案" or "点击应用天赋方案",aliases={"天赋","方案","talents","loadout"},
            payload={configID=id,specID=spec},actions={"apply"}},info.name.."\0"..spec.."\0"..tostring(current))
        checkpoint()
    end
end
local function applyTalent(entry)
    if specID()~=entry.payload.specID then return {ok=false,message="专精已改变，请重新搜索"} end
    local found=false
    ...
    if not found then return {ok=false,message="天赋方案已删除"} end
...
I.Builtin.TalentLoadouts=C:New("builtin.talent-loadouts","天赋方案",
    {"TRAIT_CONFIG_UPDATED","TRAIT_CONFIG_LIST_UPDATED","PLAYER_SPECIALIZATION_CHANGED"},talents,{apply={title="应用方案",run=applyTalent}})
...
local function equipment(_,put,checkpoint)
    ...
    for _,id in ipairs(ids) do
        ...
        local subtitle=equipped and "当前装备方案" or ((lost or 0)>0 and "部分装备缺失" or "点击装备方案")
        put({id="equipment:"..id,title=name,kind="equipment-set",kindTitle="装备方案",icon=icon,
            subtitle=subtitle,aliases={"装备","方案","equipment","gear"},payload={setID=id},actions={"equip"}},
            name.."\0"..tostring(icon).."\0"..subtitle)
        checkpoint()
    end
end
I.Builtin.EquipmentSets=C:New("builtin.equipment-sets","装备方案",{"EQUIPMENT_SETS_CHANGED","PLAYER_EQUIPMENT_CHANGED"},equipment,
    {equip={title="装备方案",run=function(entry)
        local name=C_EquipmentSet.GetEquipmentSetInfo(entry.payload.setID)
        if not name then return {ok=false,message="装备方案已删除"} end
        local ok=C_EquipmentSet.UseEquipmentSet(entry.payload.setID)
        return {ok=ok==true,close=ok==true,message=not ok and "无法装备此方案" or nil}
    end}})
