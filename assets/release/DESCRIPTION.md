...
魔兽世界万用搜索器。Alt + 空格启动，输入你想找的内容，快速获取搜索结果，直接施放技能、使用物品或打开设置。

### 详情正文

#### 荔枝启动器

背包里的炉石、藏在菜单里的设置，都可以直接搜名字找。按 Alt + 空格打开荔枝，搜“炉石”就能点击使用，输入“主音量”就能打开暴雪设置并定位。
...
Find your Hearthstone without opening your bags, or locate a setting without hunting through menus. Press Alt + Space, search for Hearthstone and click to use it. Search for `master volume` to open and locate the native setting.
...
- `master volume`: Open and locate the native Blizzard setting. Right-click to set an alias or pin it to the home page.
