...
The current addon distributes the replacement `addon/Lychee/Media/MenuIcons/*.tga` artwork and its provenance note. The legacy IconPark references and derived previews remain under Apache-2.0 in this repository; see [third-party notices](../../THIRD_PARTY_NOTICES.md). If rebuilding or distributing those legacy assets, retain their Apache license and attribution. No SVG renderer, fonts, npm package, or reference source vectors ship to the game.
