...
for product in ('Classic','Titan','Anniversary'):
    ...
    if not toc.exists():
        ...
        continue
    assert 'Providers/LDT/' not in toc.read_text()
manifest=json.loads((ROOT/'tools/client_manifest.json').read_text())
...
for toc in (ROOT/'addon/Lychee').glob('Lychee*.toc'):
    ...
    if toc.name not in ('Lychee.toc','Lychee_Mainline.toc'): assert 'Providers/LDT/' not in text
    assert 'Providers/MDT/' not in text
...
probe = '''
LycheeInternal={ProviderModules={}}
dofile("addon/Lychee/Providers/LDT/Data.lua")
local M=LycheeInternal.ProviderModules.LDT
