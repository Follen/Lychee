...
-- lua tests/performance/performance_memory.lua [--check|--stress]
...
print=function() end
dofile("tests/support/package_providers.lua")
...
if arg[1]=="--stress" then
    ...
    assert(dofile("tests/support/provider_fixture.lua"):Register({id="stress.spells",title="压力技能",version="1.0.0",apiVersion="1.0.0",catalog=spells}))
end
collectgarbage("collect")
...
print(string.format("MEMORY entries=%d providers_KiB=%.1f combined_KiB=%.1f 48_queries_alloc_KiB=%.1f retained_growth_KiB=%.1f query_mean_ms=%.3f",count,bosses,retained,allocated,growth,ms/48))
