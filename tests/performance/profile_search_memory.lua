...
-- lua tests/performance/profile_search_memory.lua [mounts|builtins] [seen|memberships|cache|saved|updates]
...
local path=scenario=="mounts" and "tests/providers/mounts_vault_smoke.lua" or "tests/providers/providers_smoke.lua"
...
dofile=function(file)
    ...
    if arg[4] and file=="addon/Lychee/Providers/Mounts/Provider.lua" then return originalDofile(arg[4]) end
...
if scenario=="mounts" then
    C_Timer=nil
    assert(LycheeInternal.ProviderModules.Mounts:Init())
