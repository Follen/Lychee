...
local handle=assert(Fixture:Register(definition("ingestion.shared",input)))
...
local function current(id) return privateIndex:GetRecord(source,id or "a") end
...
calls=0
assert(handle.catalog:Update({upsert={record("a")}}))
...
local function rejected(value, code)
    ...
    local ok,result,err=pcall(handle.catalog.Update,handle.catalog,{upsert={value}})
...
assert(not handle.catalog:Update({replace=sparse}),"sparse batches remain invalid")
local cycle={};cycle[1]=cycle;assert(not handle.catalog:Update({replace=cycle}))
...
issecretvalue=function(value)
    if value=="reenter trigger" and not reentered then
        reentered=true
        inner=assert(Fixture:Register(definition("ingestion.inner",{record("inside")})))
    end
    return value==secret
end
bad=record("a");bad.payload.nested.value="reenter trigger"
assert(handle.catalog:Update({upsert={bad}}) and reentered and inner)
assert(inner:Unregister())
issecretvalue,canaccessvalue,canaccesstable=oldSecret,oldAccess,oldTable

-- No caller-provided ownership tag can bypass the public query boundary.
assert(Lychee.ReceiveRecords==nil and Lychee._HasRecordReceipt==nil)
assert(I.Registry._OwnRecords==nil)
...
assert(handle.catalog:Update({upsert={left,right}}))
...
for n=1,600 do
    ...
    assert(handle.catalog:Update({upsert={value}}))
end
local entry=catalogState
...
assert(handle.catalog:Update({upsert={large}}) and #current().aliases[1]==1200,"large metadata remains intact")
