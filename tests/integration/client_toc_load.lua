    ...
    for _,locale in ipairs({"enUS","zhCN"}) do
        ...
        if client[4]~="retail" then
            for _,name in ipairs({"Crests","Keystones","Bosses","GreatVault"}) do
                assert(not loaded["Providers/"..name.."/Provider.lua"],"retail module leaked into other TOC: "..name)
            end
            assert(not loaded["Providers/Bosses/JournalCatalog.lua"])
        end
        ...
        local restored={}
        for _,definition in ipairs(I.ProviderModules.Definitions) do
        ...
        calls={};I.ProviderModules._initialized=nil
        I.ProviderModules:Init();I.ProviderModules:Init()
        for _,definition in ipairs(I.ProviderModules.Definitions) do
            local allowed=I.ProviderModules.Support:Available(definition,client[4])
            assert(calls[definition.module]==(allowed and 1 or nil),definition.id)
            assert(not I.ProviderModules.Support:Available(definition,"unknown"))
            if allowed then
                for _,symbol in ipairs(definition.requires) do
                    local namespace,key=owner(symbol);local previous=namespace[key];namespace[key]=nil
                    assert(not I.ProviderModules.Support:Available(definition,client[4]),symbol)
