...
local ledger=I.ProviderModules.CatalogLedger:New()
...
assert(ledger:Reconcile(handle,{a="A",b="B"},build,{full=true}))
assert(commits==1 and ledger.values.a=="A")
assert(ledger:Reconcile(handle,{a="A",b="B"},build,{full=true}) and commits==1)
assert(ledger:Reconcile(handle,{a=false},build) and not ledger.values.a and ledger.values.b=="B")
failAt=commits+1
local ok,err=ledger:Reconcile(handle,{c="C"},build,{full=true})
assert(not ok and err.code=="TEST_FAILURE" and ledger.values.b=="B" and not ledger.values.c)
assert(I.Providers.entries[handle.id].recordMap.b and not I.Providers.entries[handle.id].recordMap.c)
failAt=nil
assert(ledger:Reconcile(handle,{c="C"},build,{full=true}))
...
ok,err=ledger:Reconcile(handle,desired,build,{full=true,batchSize=16})
...
assert(ledger:Reconcile(handle,desired,build,{full=true,batchSize=16}))
assert(commits-before==2 and largest<=16)
local broken={bad="Bad"}
ok,err=ledger:Reconcile(handle,broken,function() error("build failed") end)
assert(not ok and err.code=="CATALOG_BUILD_FAILED" and not ledger.values.bad)
assert(ledger:Reconcile(handle,broken,build),"a failed build releases the operation")
...
assert(ledger:Reconcile(handle,replacement,build,{full=true,batchSize=16}))
-- A caller can abandon a yielded diff; its successor must not stay busy.
local cancelled
local job=coroutine.create(function()
    local accepted,why=ledger:Reconcile(handle,{cancelled="Cancelled"},build,{checkpoint=coroutine.yield})
...
assert(ledger:Reconcile(handle,{successor="Successor"},build))
...
ok,err=ledger:Reconcile(handle,{retired="Retired"},build)
...
local compact=I.ProviderModules.CatalogLedger:New({
...
assert(compact:Reconcile(handle,{[12]={title="Named",temporary="discard"}},named,{full=true}))
assert(compact.values[12].title=="Named" and compact.values[12].temporary==nil)
assert(compact:Reconcile(handle,{[12]=false},named) and not next(compact.values))
assert(handle:Unregister())
