...
slow:Reset()
-- Replay a click-through cursor ring, including its native GCD/texture child.
-- Positions are screen pixels; the tested frame uses a different UI scale.
do
    local ring=observation("Frame")
    ring.GetFrameStrata=known("TOOLTIP");ring.IsMouseEnabled=known(false)
    ring.GetScript=known(function() error("inspector must never execute foreign scripts") end)
    ring.GetEffectiveScale=known(0.5)
    local texture=observation("Texture",ring)
    texture.GetEffectiveScale=known(0.5)
    ring.GetRegions=known(texture)
    local below=observation("Texture")
    below.GetRect=known(0,0,2000,2000)
    local snapshot={ring,texture,below}
    local tested=Picker.New(allowed,observed,function() return 0 end)
    local function move(x,y)
        ring.GetRect=known((x-25)*2,(y-25)*2,100,100)
        texture.GetRect=ring.GetRect
    end
    move(50,50)
    assert(tested:Step(ring,snapshot,50,50).object==ring,"stationary ring is not yet motion evidence")
    move(60,50)
    assert(tested:Step(ring,snapshot,60,50).object==ring,"one motion observation is not confirmation")
    for i=1,5 do assert(tested:Step(ring,snapshot,60,50).object==ring,"stationary polls must not accumulate movement") end
    move(80,65)
    assert(tested:Step(texture,snapshot,80,65).object==below,"confirmed cursor decoration and children must not steal the underlying hit")
    assert(tested:Describe():find("cursor%-follower"),"report explains the motion-based exclusion")
    assert(not tested:Step(ring,{ring,texture},80,65).object,"cursor ring over empty world yields no target")
    assert(tested:Step(ring,snapshot,80,65).object==below,"stationary pause retains confirmed follower")
    -- Moving over the same stationary noninteractive ring revokes exclusion.
    assert(tested:Step(ring,snapshot,90,65).object==ring,"stopped follower must become eligible again")
    tested:Reset()
    move(50,50)
    assert(tested:Step(ring,snapshot,50,50).object==ring,"reset discards motion history")
    for _,key in ipairs({"IsMouseEnabled","GetScript","GetFrameStrata"}) do
        local saved=ring[key]
        for _,entry in ipairs({known(true),{"secret"},{"error"},known()}) do
            tested:Reset();ring[key]=entry
            for i=1,4 do
                local x=50+i*10;move(x,50)
                assert(tested:Step(ring,snapshot,x,50).object==ring,"ineligible/unreadable "..key.." must never prove decoration")
            end
        end
        ring[key]=saved
    end
    -- Tiny input accumulates into meaningful motion; alternating directions
    -- cannot be mistaken for a frozen-position coincidence.
    tested:Reset();move(50,50);tested:Step(ring,snapshot,50,50)
    for x=51,58 do move(x,50);tested:Step(ring,snapshot,x,50) end
    assert(tested:State().object==below,"slow cursor motion still confirms follow behavior")
    local beforeFrozen=tested:Step(ring,snapshot,58,50)
    move(300,300)
    assert(tested:Step(ring,snapshot,300,300,true).object==beforeFrozen.object,"frozen inspection does not collect motion")
    tested:Reset()
    -- A fixed overlay with an unrelated update script must remain pickable.
    move(50,50)
    for x=40,60,4 do assert(tested:Step(ring,snapshot,x,50).object==ring,"stationary overlay is not cursor decoration") end
    -- Validate the cache bound and reset ownership, including overflow/re-entry.
    for i=1,20 do
        local extra=observation("Frame")
        extra.GetFrameStrata=known("TOOLTIP");extra.IsMouseEnabled=known(false);extra.GetScript=ring.GetScript
        extra.GetRegions=known(texture)
        tested:Step(extra,{extra},50,50)
    end
    local count=0;for _ in pairs(tested.followers) do count=count+1 end
    assert(count<=8 and #tested.followerSlots<=8,"motion records have bounded ownership")
    tested:Reset()
    assert(not next(tested.followers) and not next(tested.followerSlots),"reset releases all tracked roots")
    -- Mutable observations avoid attributing fixture table allocations to Picker.
    local coords=known(50,50,100,100)
    ring.GetRect=coords;texture.GetRect=coords
    local function sample(i)
        local x,y=50+(i%50)*5,50+math.floor(i/50)%4*5
        coords[2],coords[3]=(x-25)*2,(y-25)*2
        return tested:Step(ring,snapshot,x,y)
    end
    for i=1,10 do sample(i) end
    collectgarbage("collect");local base=collectgarbage("count");collectgarbage("stop")
    local start,maxSample=os.clock(),0
    for i=11,1010 do
        local began=os.clock()
        assert(sample(i).object==below,"continuous motion keeps underlying target stable")
        maxSample=math.max(maxSample,(os.clock()-began)*1000)
    end
    local ms,allocated=(os.clock()-start)*1000,collectgarbage("count")-base
    collectgarbage("restart");collectgarbage("collect")
    local retained=math.max(0,collectgarbage("count")-base)
    assert(allocated<64 and retained<16 and maxSample<4,"cursor motion work stays within the declared budget")
    print(string.format("CursorMotion1000 cpu_ms=%.2f max_sample_ms=%.2f allocated_KiB=%.1f retained_KiB=%.1f",ms,maxSample,allocated,retained))
    tested:Reset()
end
print("Addon inspector picker PASS observed unknown/negative/native authority/no expansion/cursor motion + "..comparisons.." reference comparisons")
