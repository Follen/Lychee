...
do
    ...
    assert(page.prefixInput:GetText()~="草稿" and not page.save.enabled)
    edit(page.prefixInput,"范围");edit(page.keywordInput,"展示");page.save.frame.scripts.OnClick()
    local function search(q) local _,items=LycheeInternal.Search.Query:Query(q,{visible=true});return items end
    assert(#search("目标")==1 and #search("范围:目标")==1 and #search("展示")==1,"all three routes coexist")
    page.globalToggle.scripts.OnClick();page.save.frame.scripts.OnClick()
    assert(#search("目标")==0 and #search("范围:目标")==1 and #search("展示")==1,"global off retains shortcuts")
    edit(page.prefixInput,"");edit(page.keywordInput,"");page.save.frame.scripts.OnClick()
    assert(page.error:GetText()~="" and #search("展示")==1,"cannot save an unreachable provider")
    page.reset.frame.scripts.OnClick();assert(not policy:Configuration("manage.ui",page.entry.definition))
    page.save.frame.scripts.OnClick();assert(policy:Configuration("manage.ui",page.entry.definition))
    local before=createdFrames
    ...
    edit(page.keywordInput,"过期");page.save.frame.scripts.OnClick()
    ...
    assert(not page.globalToggle:IsShown() and not page.prefixInput:IsShown() and not page.save.enabled and not page.reset.enabled)
