...
OUTPUT = ROOT / "addon/Lychee/Builtin/LDT/Data.lua"
...
def render(data):
    ...
    lines = ["-- Generated by tools/build_enemy_catalog.py from assets/data/enemies.json.",
             "-- Named factual records. Materialized only when a query or detail needs them.",
             "local M={id=\"builtin.ldt\"}", "_G.LycheeInternal.Builtin.LDT=M",
             "M.dungeonIDs="+lua([d["id"] for d in data["dungeons"]]), "local loaders={"]
    lines += ["    ["+str(d["id"])+"]=function() return "+lua(d)+" end," for d in data["dungeons"]]
    lines += ["}", "function M:LoadDungeon(id)", "    local load=loaders[id]", "    return load and load()", "end", ""]
    return "\n".join(lines)
