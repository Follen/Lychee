...
顶部左侧为原始标志，中间为 48 高搜索区，右侧为红色 Esc 关闭按钮，无 tooltip。空输入时最近使用最多五项排成一行，单项布局区域 112 × 96，间隔 8；图标上、最多两行的名称下。选中只着色图标周围 48 × 48 区域，不为整个入口绘制卡片底色。
...
- **结果行**：默认与背景融合，唯一当前项呈深红底与细描边；鼠标进入另一项会移动当前选择，方向键操作同一个选择。有次要操作时，仅当前项显示按钮。原生悬停提示分为亮色标题、弱化类型、独立说明、红色主操作及可用拖动提示；不显示匹配百分比等搜索诊断。
...
- **动作菜单**：搜索行、最近使用图标和安全技能覆盖层均可右键访问全部已声明动作。普通动作按声明执行；选择次要技能动作后先准备对应安全按钮，需再次真实点击才能施放。菜单关闭或结果变化后，旧菜单不能继续操作旧结果。
