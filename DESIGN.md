...
- **结果行**：默认与背景融合，悬停和选择呈深红底与细描边；有次要操作时，仅在悬停或选中时显示按钮。原生悬停提示分为亮色标题、弱化类型、独立说明、红色主操作提示；不显示匹配百分比等搜索诊断。
- **最近使用**：空输入显示成功使用过的入口；没有记录时显示引导文字。分组标题使用小号灰字，在滚动内容内部留出顶部 10、左侧 12，避免依赖滚动子框的预设偏移。
